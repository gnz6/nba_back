const { reviewModel,productModel, usersModel } = require("../models")
...
const createReview = async (req, res) => {
    try {
        ...
        await productModel.findOneAndUpdate({_id:post_id},{rating:prom})
...
const updateReview = async (req, res) => {
    try {
        ...
        let suma = 0;
         reviews.forEach(e=> {suma = suma + (e.value?e.value:0)})
          suma = suma + value
        await productModel.findOneAndUpdate({_id:post_id}, {rating:suma/reviews.length+1})
...
const deleteReview = async (req, res) => {
    try {
       ...
       let suma = 0;
        reviews.forEach(e=>{ suma = suma + (e.value?e.value:0)})
       await productModel.findOneAndUpdate({_id:post_id}, {rating:suma/reviews.length})
